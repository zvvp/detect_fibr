...
impl TimeParam {
    pub fn new() -> TimeParam {
        ...
        debug!("get_inds_min_diff");
        time_param.get_clear_intervals();
        debug!("get_clear_intervals");
        ...
        for (_i, res_line) in reader.lines().enumerate() {
            ...
            if line.contains(';') {
                ...
                if split_line.len() == 3 {
                    ...
                    let char_in_line = split_line[2].chars().nth(0);
    ...
    fn get_clear_intervals(&mut self) {
        ...
        for i in 2..self.chars.len() - 2 {
            ...
            if self.chars[i] == 'V' {
                ...
                + (self.intervals[i] - self.intervals[i + 2]).abs()
                < 30.0
    ...
    // fn get_clear_intervals1(&mut self) {
    //     let mut intervals = self.intervals.clone();
    ...
    fn get_inds_min_diff(&mut self) {
        ...
        */
        ...
        debug!("inds_min_diff_len = {}", self.inds_min_diff.len());
    }
}
